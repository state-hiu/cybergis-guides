...
This guide provides instructions for creating and managing OSM extracts via GeoServer/GeoGig.  First, this guide includes sections on [provision](#provision), [installation](#installation), and [running GeoServer](#running-geoserver).  The guide then includes sections on [initializing](#initializing-extracts), [updating](#updating-extracts), [snapshotting](#snapshotting-extracts), and [animating](#animating-extracts) extracts.

Please see the CyberGIS OSM Extractions repo for examples at [https://github.com/state-hiu/cybergis-osm-extractions](https://github.com/state-hiu/cybergis-osm-extractions).
