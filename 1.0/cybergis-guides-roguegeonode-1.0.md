...
Also, you may need to disable GZIP compression, depending on network configuration.  To do so, comment out the `GZIP Compression Filter` in `/var/lib/tomcat7/webapps/geoserver/WEB-INF/web.xml` and `/var/lib/geonode/geoshape/geoserver_ext/src/main/webapp/WEB-INF/web.xml.`.
